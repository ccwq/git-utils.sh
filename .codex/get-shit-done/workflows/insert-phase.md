...
```bash
INIT=$(node "D:/project/git-utils.sh/.codex/get-shit-done/bin/gsd-tools.cjs" init phase-op "${after_phase}")
...
```bash
RESULT=$(node "D:/project/git-utils.sh/.codex/get-shit-done/bin/gsd-tools.cjs" phase insert "${after_phase}" "${description}")
```
