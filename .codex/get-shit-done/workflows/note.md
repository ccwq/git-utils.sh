...
- **Global scope**: `D:/project/git-utils.sh/.codex/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/project/git-utils.sh/.codex/notes/`)
...
2. Glob `D:/project/git-utils.sh/.codex/notes/*.md` (if directory exists) — global notes
...
Global (D:/project/git-utils.sh/.codex/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/project/git-utils.sh/.codex/notes/` — works in any directory
