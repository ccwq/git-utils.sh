...
```bash
INIT=$(node "E:/project/self.project/git-utils.sh/.claude/get-shit-done/bin/gsd-tools.cjs" init phase-op "${after_phase}")
...
```bash
RESULT=$(node "E:/project/self.project/git-utils.sh/.claude/get-shit-done/bin/gsd-tools.cjs" phase insert "${after_phase}" "${description}")
```
